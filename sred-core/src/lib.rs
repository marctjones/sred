...
//!
//! `Box<dyn Fn…>` extension hooks (tokens, spellcheck, fragment renderer) and a
//! few wide renderer signatures trip `type_complexity`/`too_many_arguments`;
//! those are allowed crate-wide as idiomatic for this surface.
...
mod tests {
    ...
    fn markdown_roundtrip_stable() {
        ...
        assert_eq!(
            doc, doc2,
            "serialize→parse must be a fixpoint\n--- out ---\n{out}"
        );
    }
    ...
    fn typst_roundtrip_stable() {
        ...
        assert_eq!(
            doc, doc2,
            "typst serialize→parse fixpoint\n--- out ---\n{out}"
        );
    }
    ...
    fn live_preview_hides_markers_off_caret_line() {
        ...
        assert!(
            display.contains("• item"),
            "bullet should be substituted: {display:?}"
        );
    ...
    fn live_preview_reveals_markers_on_caret_line() {
        ...
        assert!(
            display.contains("## Heading"),
            "caret line shows the raw marker: {display:?}"
        );
    ...
    fn markdown_inline_via_commonmark() {
        ...
        fn marks_of(line: &str) -> Vec<MarkSet> {
            ...
            spans
                .iter()
                .flat_map(|s| s.text.chars().map(move |_| s.marks))
                .collect()
        }
        ...
        // Inline link.
        assert!(marks_of("see [t](u)")
            .iter()
            .any(|x| x.contains(MarkSet::LINK)));
        // GFM strikethrough.
    ...
    fn typst_live_preview_projects_markup() {
        ...
        assert!(
            display.contains("• item"),
            "typst '- ' should become a bullet: {display:?}"
        );
    ...
    fn typst_inline_marks_bold_italic_math() {
        ...
        assert!(
            spans.iter().any(|s| s.marks.contains(MarkSet::BOLD)),
            "typst *…* is bold"
        );
        assert!(
            spans.iter().any(|s| s.marks.contains(MarkSet::ITALIC)),
            "typst _…_ is italic"
        );
        assert!(
            spans.iter().any(|s| s.marks.contains(MarkSet::CODE)),
            "typst $…$ styled like code"
        );
    }
    ...
    fn typst_full_via_typst_syntax() {
        ...
        assert!(marks_of("#let x = 1")
            .iter()
            .any(|x| x.contains(MarkSet::CODE)));
        // A reference @target is link-styled.
        assert!(marks_of("see @intro here")
            .iter()
            .any(|x| x.contains(MarkSet::LINK)));
    }
    ...
    fn styling_cache_matches_fresh_computation() {
        ...
        for tokens_gen in [0u64, 7] {
            let toks: &[view::TokenSpec] = if tokens_gen == 0 {
                &[]
            } else {
                std::slice::from_ref(&spec)
            };
            for caret in [0usize, 1, 4, 5] {
                for t in seq {
                    ...
                    let fresh =
                        view::styled_runs(t, Format::Markdown, 16.0, caret, toks, tokens_gen);
